class Tags::ShowPage < MainLayout
  ...
  needs tag : Tag
  needs tags : TagQuery
  needs questions : Array(Question)
      ...
      div class: "w-full max-w-6xl mx-auto" do
        div class: "grid gap-2 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-3" do
          div class: "sm:col-span-1 md:col-span-3 align-baseline" do
            h1 "Questions tagged: #{tag.name} ", class: "text-2xl text-gray-800 font-bold align-baseline"
            small "Currently viewing questions in #{tag.name}."
          end
          latest_questions
          right_sidebar
        end
          ...
          div class: "flex justify-between items-center mt-4" do
            ...
            div class: "flex items-center" do
              if !current_user.profile_picture_path.nil?
                img alt: "avatar", class: "mx-4 w-10 h-10 object-cover rounded-full hidden sm:block", src: "/assets/uploads/#{question.author.profile_picture_path.not_nil!}"
              end
